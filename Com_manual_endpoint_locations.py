...
def manual_endpoint_location(): 
    ...
    while not rospy.is_shutdown(): 
        ...
        try: 
            ...
            grip_in_ang = input('\nInput angle to turn the gripper (degree): ')
        except: 
        ...
        grip_rads_ang = np.radians(grip_in_ang) # Convert angle to radians
        grip_ang_clip = np.clip(grip_rads_ang, np.min(rotlim_gripper), np.max(rotlim_gripper)) # Clip the gripper so it's in the possible range
        print("Angle to turn gripper", grip_ang_clip)

        # If the program gets here it has been told to go ahead. 
        # Move to endpoint. 
        ang_lim = np.append(ang_lim, grip_ang_clip) ## append a Gripper angle. This needs updating. 
