...
movement_time = round(1000/command_frequency)
...
def main(): 
    ...
    command_bus_servos(servo_indices_base_to_tip , servo_neutral_cmds_base_to_tip, duration=1000)
...
def move_servos_and_set_joint_state(msg_in):
    ...
    if arm_is_present:
        command_bus_servo(0,int(cmd_00))
        command_bus_servo(1,int(cmd_01))    
        command_bus_servo(2,int(cmd_02))    
        command_bus_servo(3,int(cmd_03))    
        command_bus_servo(4,int(cmd_04))    
        command_bus_servo(5,int(cmd_05))    
        command_bus_servo(6,int(cmd_06))    

    global joint_state_msg
    try:
        joint_state_msg.position = xarm_model.convert_servo_commands_to_joint_state(cmd_all, arm_is_present)
        joint_state_msg.name = ['base_joint', 'shoulder_joint', 'elbow_joint', 'forearm_joint', 'wrist_joint', 'fingers_joint', 'gripper']
...
def command_bus_servo(servo_index, cmd, duration=movement_time):
    servo_ID = servo_IDs_base_to_tip[servo_index]
    arm.setPosition([ [servo_ID, cmd] ], duration=duration )

# Function to command all Hiwonder Bus Servos with individual commands
def command_bus_servos(servo_indices, cmds, duration=movement_time):
    servo_IDs = [servo_IDs_base_to_tip[x] for x in servo_indices]
    arm.setPosition( list(zip(servo_IDs, cmds)), duration=duration )


# Function to shut down all the servos by sending them the shutdown command.
def shutdown_arm():
    arm.servoOff()
    print('Arm is Powered Down.')
